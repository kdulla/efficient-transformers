...
qeff_model_blocked = QEFFAutoModelForImageTextToText.from_pretrained(
    model_id, attn_implementation="eager", kv_offload=True, config=config, qaic_config=qaic_config,
)
...
if skip_vision:
    ...
    inputs_blocked = qeff_model_blocked.model.prepare_inputs_for_generation(inputs=inputs_processed, prefill_seq_len=128, batch_size=batch_size)
    output_blocked = qeff_model_blocked.generate(inputs=inputs_blocked, generation_len=100)
    print(output_blocked.generated_ids)
    print(processor.tokenizer.batch_decode(output_blocked.generated_ids))
    print(output_blocked)
